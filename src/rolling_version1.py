...
def main(contigs,query_path,k,test,fix,fout,tout,fixedout,database,thre,rolling=12):
    try:
        #t = Timer()
        threshold = thre
        db = database
        ...
        for seqname,seq in seq_dict.items():
            ...
            while i < len(seq)-k+1: #k is 25 : 
                ...
                if occurrance < threshold:
                    ...
                    for temp in range(max(0,j-rolling),j):
                    ...
                    print("previous good kmer occurances:",end = " ")
                    print(count_record)
                    ...
                    if len(count_record) > 0:
                        while  qf[jf.MerDNA(seq[good_before-k+2:good_before+2]).get_canonical()] >= 0.5*sum(count_record)/len(count_record) and good_before-k+1 < good_after:
                            if good_before == -1:
                                break
                            count_record = count_record[1:]
                            count_record.append(qf[jf.MerDNA(seq[good_before-k+2:good_before+2]).get_canonical()])
                            print(qf[jf.MerDNA(seq[good_before-k+2:good_before+2]).get_canonical()],end= " ")
                            good_before +=1
            ...
            for temp in wrong_kmers_list:
                print(qf[jf.MerDNA(seq[temp:k+temp]).get_canonical()],end = " ")
